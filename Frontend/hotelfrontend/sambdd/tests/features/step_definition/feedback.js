...
Given("I login and review my favourite", async () => {
  ...
  const feedbackRes = await axios.post(
    `http://localhost:3005/ehotel/guest/${GuestID}/hotels/${hotelID}/feedback`,
    {
      feedbacks: "Best hotel ever....Love the services..",
      rating: "3",
    },
    guestConfig
  );

  expect(feedbackRes.data.feedbacks).to.be.equal(
    "Best hotel ever....Love the services.."
  );
});
